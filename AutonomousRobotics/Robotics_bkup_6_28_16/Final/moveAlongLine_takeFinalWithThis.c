int moveForward(int speed, int ticks) {
	...
	printf("moveForward, ticks: %d, speed: %d\n", ticks, speed);
...
int main() {
	...
	int didTurnLeft = 0;
	int didTurnRight = 0;
	int didUseRightTurnCorrection = 0;
	int didUseLeftTurnCorrection = 0;
	//end flags
	...
	while(keepRunning == 1) {
		...
		//now to move forward if a correction turn wasn't given

		if(didTurnLeft == 0 && didTurnRight == 0) {
			moveForward(500, ticksInc);
		}
		
		//here I will do some checking to see if an extra correction turn is needed
		if(didTurnRight == 1 && didTurnLeft == 0) {

		}
		else if(didTurnRight == 0 && didTurnLeft == 1) {
