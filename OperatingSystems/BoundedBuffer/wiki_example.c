...
//I got some of this code from https://en.wikipedia.org/wiki/Producer–consumer_problem
...
int count = 0; //this keeps track of the next "slot" in the bounded buffer to allow a new item to be placed in the buffer
...
int inc = 9; // I use this to be the number to add to the buffer. This is easier to debug than a random number!
void * producer(void * arg) {
	int temp = (long)arg;
	while (TRUE) {
...
void putItemIntoBuffer(int arg) { //this whole section of code is "protected" my a mutex semaphore
	inc++;
	item = inc;
	buffer[count] = item;
	count++;
...
int removeItemFromBuffer(int arg) { //this whole section of code is "protected" a mutex semaphore
